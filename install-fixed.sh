...
apt-get update -qq
apt-get install -y -qq ca-certificates curl git jq lsb-release ufw openssl apt-utils > /dev/null
...
# Docker daemon'ı başlat ve bekle
log_info "Docker daemon başlatılıyor..."
systemctl start docker

# Docker daemon'ın hazır olmasını bekle
log_info "Docker daemon'ın hazır olması bekleniyor..."
for i in {1..30}; do
  if docker info >/dev/null 2>&1; then
    log_info "Docker daemon hazır!"
    break
  fi
  log_info "Docker daemon bekleniyor... ($i/30)"
  sleep 2
done
