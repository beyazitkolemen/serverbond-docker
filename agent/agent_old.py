...
def get_php_versions(x_agent_token: str = Header(None)):
    protect(x_agent_token)
    return {
...
def get_base_system_status(x_agent_token: str = Header(None)):
    protect(x_agent_token)
    try:
        ...
        for container in base_containers:
            container_statuses[container] = get_container_status(container)
...
def restart_base_system(x_agent_token: str = Header(None)):
    protect(x_agent_token)
    try:
...
def stop_base_system(x_agent_token: str = Header(None)):
    protect(x_agent_token)
    try:
        base_dir = Path(CONFIG["base_system"]["shared_services_dir"])
        result = subprocess.run(
...
def start_base_system(x_agent_token: str = Header(None)):
    protect(x_agent_token)
    try:
        base_dir = Path(CONFIG["base_system"]["shared_services_dir"])
        result = subprocess.run(
...
def update_systemd_service(x_agent_token: str = Header(None)):
    protect(x_agent_token)
    try:
...
def get_systemd_status(x_agent_token: str = Header(None)):
    protect(x_agent_token)
    try:
